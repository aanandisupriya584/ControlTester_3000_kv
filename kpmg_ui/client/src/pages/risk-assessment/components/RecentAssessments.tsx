...
type RecentAssessmentsProps = {
  ...
  onDeleteAssessment: (assessment: RiskAssessment) => void;
  getAssessmentAppCount: (assessment: RiskAssessment) => number;
  formatAssessmentDate: (value?: string | null) => string;
};

// Shows saved assessments and forwards open/delete actions back to the parent page.
    ...
    <div className="flex min-h-0 flex-col overflow-hidden rounded-[8px] border border-[#D8E0ED] bg-white">
      ...
      {isLoading ? (
            ...
            >
              ...
              >
                ...
                <p className="mt-1.5 text-[12px] text-[#5A6478]">
                  {getAssessmentAppCount(assessment)} application{getAssessmentAppCount(assessment) === 1 ? "" : "s"} - Updated {formatAssessmentDate(assessment.updated_at)}
                </p>
              </button>
        ...
        <div className="px-6 py-10 text-center">
          ...
          <NewButton className={primaryButtonClassName} extraClassName="mt-6" onClick={onOpenNew} />
        </div>
      )}
    </div>
  );
